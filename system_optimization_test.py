...
async def test_text_validator():
    ...
    for title, expected_min_score, should_pass in test_cases:
        ...
        logger.info(f"标题: '{title}' -> 有效: {is_valid}, 得分: {score:.2f}, 原因: {reason}")
...
async def test_progress_tracker():
    ...
    if final_progress:
        logger.info(f"最终状态: {final_progress.status.value}, "
...
async def test_cache_manager():
    ...
    cached_search = await cache_manager.get_search_results("测试关键词", [1, 2])
...
async def test_epub_generator():
    ...
    chapters = [
        Chapter(title="第一章 开始", content="这是第一章的内容。\n\n这是第二段。", order=1),
        Chapter(title="第二章 发展", content="这是第二章的内容。\n\n情节开始发展。", order=2),
        Chapter(title="第三章 结束", content="这是第三章的内容。\n\n故事结束了。", order=3)
    ]
...
async def test_search_optimization():
    ...
    for keyword in test_keywords:
        try:
            ...
            end_time = time.time()
            logger.info(f"搜索完成: 找到 {len(results)} 个结果，耗时 {end_time - start_time:.2f} 秒")
