...
from app.utils.performance_monitor import performance_monitor, monitor_performance
...
async def optimized_search_novels(
    ...
    try:
        async with performance_monitor.monitor_operation(
            "search_novels", 
            {"keyword": keyword, "max_results": maxResults}
        ):
            logger.info(f"开始优化搜索，关键词：{keyword}，maxResults={maxResults}")
            
            # 使用优化版搜索服务
            results = await optimized_service.optimized_search(keyword, max_results=maxResults)
            
            end_time = time.time()
            duration_ms = (end_time - start_time) * 1000
            
            logger.info(f"优化搜索完成，找到 {len(results)} 条结果，耗时 {duration_ms:.1f}ms")
...
async def optimized_download_novel(
    ...
    try:
        ...
        async with performance_monitor.monitor_operation(
            ...
            # 使用优化版下载服务
            file_path = await optimized_service.optimized_download(url, sourceId, format, task_id)
            ...
            logger.info(f"优化下载完成，文件：{file_path}，大小：{file_size} 字节，耗时 {duration_ms:.1f}ms")
...
async def get_optimized_sources():
    ...
    try:
        async with performance_monitor.monitor_operation("get_sources"):
            ...
            logger.info(f"获取书源列表成功，共 {len(sources)} 个书源，耗时 {duration_ms:.1f}ms")
