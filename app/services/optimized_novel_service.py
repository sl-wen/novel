...
class OptimizedNovelService:
        ...
        for rule_file in rules_path.glob("*.json"):
            try:
                ...
                if source_id:
                    ...
                    logger.info(f"加载书源: {rule_data.get('name', f'Source-{source_id}')} (ID: {source_id})")
    ...
    async def _validate_sources(self):
        ...
        # 并发检查所有书源
        tasks = [check_with_semaphore(sid, source) for sid, source in self.sources.items()]
    ...
    async def optimized_search(self, keyword: str, max_results: int = 30) -> List[SearchResult]:
        """优化版搜索小说
        ...
        # 1. 检查缓存
        cache_key = self.cache_manager._generate_cache_key("search", keyword, max_results)
        ...
        async def search_single_source_optimized(source: Source, keyword: str):
            ...
            async with search_semaphore:
                try:
                    ...
                    logger.warning(f"书源 {source.rule.get('name', source.id)} 搜索超时")
                    return []
                except Exception as e:
        ...
        # 4. 并发搜索所有书源
        tasks = [search_single_source_optimized(source, keyword) for source in searchable_sources]
        ...
        # 7. 缓存结果
        await self.cache_manager.set_search_results(cache_key, filtered_results)
    ...
    def _filter_and_sort_results_optimized(
        ...
        for result in results:
            ...
            # 计算相关性得分（优化版）
            relevance_score = self._calculate_relevance_score_fast(result, keyword_lower)
    ...
    def _calculate_relevance_score_fast(self, result: SearchResult, keyword_lower: str) -> float:
        """快速计算相关性得分"""
        score = 0.0
    ...
    async def optimized_download(self, url: str, source_id: int, format: str = "txt", task_id: Optional[str] = None) -> str:
        """优化版下载小说
    ...
    async def get_book_detail(self, url: str, source_id: int) -> Optional[Book]:
        """获取小说详情（优化版）"""
        # 检查缓存
        cache_key = self.cache_manager._generate_cache_key("book_detail", url, source_id)
