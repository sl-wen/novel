...
class EnhancedHttpClient:
    ...
    async def fetch_html(self, url: str, referer: str = None, 
                        timeout: int = None, retries: int = None) -> Optional[str]:
        """获取HTML页面内容（优化版）
        ...
        for attempt in range(retries):
            try:
                ...
                async with session.get(url, headers=headers) as response:
                    ...
                    if response.status == 200:
                        ...
                        if content and len(content) > 100:
                            ...
                            logger.warning(f"响应内容过短: {len(content) if content else 0} 字符")
                        ...
                        if redirect_url:
                            ...
                            return await self.fetch_html(redirect_url, referer=url, 
                ...
                logger.error(f"请求失败 (尝试 {attempt + 1}/{retries}): {url} - {str(e)}")
                if attempt < retries - 1:
                    await asyncio.sleep(self.retry_delay * (attempt + 1))
                    continue
    ...
    async def post_data(self, url: str, data: Any = None, json: Any = None, 
                       **kwargs) -> Optional[str]:
        """发送POST请求"""
        try:
            session = await self._get_or_create_session(url)
    ...
    async def batch_fetch(self, urls: List[str], max_concurrent: int = 10) -> List[Optional[str]]:
        """批量获取HTML内容
        ...
        return {
            **self.connection_stats,
            ...
            'success_rate': (
                ...
                max(self.connection_stats['total_requests'], 1) * 100
            )
        }
